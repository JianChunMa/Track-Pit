...
class _AddVehiclePageState extends State<AddVehiclePage> {
  ...
  List<String> _models = []; // now dynamic
  ...
  Future<void> _submit() async {
    ...
    if (user == null) {
      showClosableSnackBar(context, 'You must be signed in.');
      return;
    }
    ...
    try {
      ...
      if (!mounted) return;
      showClosableSnackBar(context, 'Vehicle added successfully!');
      Navigator.pop(context);
    } on Exception catch (e) {
      final msg =
          e.toString().contains('Plate already registered')
              ? 'This plate is already registered.'
              : 'Failed to add vehicle. Please try again.';
      if (!mounted) return;
      showClosableSnackBar(context, msg);
    } finally {
